...
We also introduce an initial value surface \(S \subset D\) of codimension 1.  Given a  function \(\phi_{0} \colon S \to \mathbb{R}\), we look for a \(\phi \coon D \to \mathbb{R}\) which satisfies the differential equation such that the
