...
** (In the infinite case, we need to augment these with an axiom of countable additivity: \(sum_{n=0}^{\infty} )  We denote the collection of all distributions on a sample space \(X\) as \(\mathcal{D}(X)\).
