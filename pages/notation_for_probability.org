...
\]
Here \(m \colon S_{Y} \times S_{X} \to [0,\infty)\) is a matrix of non-negative reals.  We denote the category of such transforms as \(\mathsf{PStoch}\).  (The first "\(\mathsf{P}\)" stands for "projective".)
**
