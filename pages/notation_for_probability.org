...
** An important property of these axioms is that they respect convexity.  That is to say, if we have \(d_{1}, d_{2} \colon E \to [0,1]\) which satisfy the axioms and form \(d_{3} \colon E \to [0,1]\) as \(d_{3}(x) = s d_{1}(x) + (1-s) d_{2}) for some \(s \in [0,1]\), then \(d_3))
