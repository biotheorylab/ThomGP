...
** Since any morphism can be factored as a product of the three special morphism discussed above, knowing how the functor behaves in these cases determines its behavior in general.  For convenience, we will write \(f^{\ast}\) as a synony, for \( \mathcal{D}(f) \) when \(f)
