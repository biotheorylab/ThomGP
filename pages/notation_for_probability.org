...
*** When \(m\) is an inclusion of sets regarded as a partial function (i.e. \(m = (D, \mathrm{id}) \colon X \to X\)), then \(D(m)\) implements conditioning on the event \(D\) --- \mathcal{D}(m)(d) = d|_
***
**
