...
** (In the infinite case, we need to augment these with an axiom of countable additivity: \(d(\bigcup_{k=0}^{\infty} x_k)  = \sum_{k=0}^{\infty} d(x_k)\) )  We denote the collection of all distributions on a sample space \(X\) as \(\mathcal{D}(X)\).
