...
** This construct combines several famliar notions from probability theory.  To unpack it, we will make use of the factorization of the morphism \(m = (D,f)  \colon X \to Y \) as \( m = g \circ h \circ i \) where:
+ \(i \colon D \to S_{X} \) is the set inclusion regarded as a partial function;
+ \(h \colon S_{X} \to \mathrm{Im} (f) \) is an epimorphism; and
+ \(g \colon )
