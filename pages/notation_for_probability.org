...
In the case of finite sample space, we can define the probabilities of outcomes in terms of sihleton sets as \(d_{j} = d(\{j\})\) and these axioms are equivalent to the conditions
+ \(\sum_{j \in S} d_{j} = 1\)
+ d(e) = \sum_{j \in e} d(\{j\})
