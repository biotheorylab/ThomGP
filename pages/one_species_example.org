...
\[\begin{aligned}
 ...
 \end{pmatrix}
 \end{aligned}\]
In order to write down such a matrix formulaion, we fix an ordering \(c^{(1)}, c^{(2)}, \ldots\) of the complexes appearing as either reactants or products in our set \(R\) of reactions.  When \(c^{(m)} \)
**
