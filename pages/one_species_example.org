...
To keep the upcoming calculations from becoming unwieldy, we will tacitly set \(k_{A \to \emptyset} = 1\) by rescaling \(t\) and write \(k\) for  We exponentiate to obtain the generating function:
