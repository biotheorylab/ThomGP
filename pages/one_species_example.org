...
is of first order, a function of \(z\) and \(t\) wil be a solution iff it is constant along characteristics.  These are determined by the equations \[ \Gamma (z, t) = f(e^{-k_{A \to \emptyset} t} (z - 1)) + \frac{k_{\emptyset \to A}}{k_{A \to \emptyset}} z. \]
