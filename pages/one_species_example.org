...
To keep the upcoming calculations from becoming unwieldy, we will tacitly set \(k_{A \to \emptyset} = 1\) by rescaling \(t\) and write \(k\) for \(k_{\emptyset \to A}\): 
\[ \Gamma = k (1 - e^{-t}) (1 - e^{\theta}) - \log (e^{-t} (e^{\theta} - 1) + 1) \]
We exponentiate to obtain the generating function:
\[Z = \exp\left(k (1 - e^{-t}) (e^{\theta} - 1)\right) \left(e^{- t} (e^{\theta} - 1) + 1\right)\]
To recover the probability distribution, expand as a power series, \(Z= \sum_{k=0}^{\infty} p_k z^k\), where
\[p_{k} = e^{-k} (1 - e^{-k_{A \to \emptyset} t})} \left( \right)\]
**** *Legendre Transform*
