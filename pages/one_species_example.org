...
** LATER To see what the solution looks like in a simple example, suppose that \(k_{2A \to 3A} = k_{3A \to 2A} = 0\) ("ID" process).  Then we may write the equation as
:PROPERTIES:
:later: 1613159768588
:END:
...
is of first order, a function of \(z\) and \(t\) will be a solution iff it is constant along characteristics.  These are determined by the equations \[ \Gamma (z, t) = f(e^{-k_{A \to \emptyset} t} (z - 1)) + \frac{k_{\emptyset \to A}}{k_{A \to \emptyset}} z. \]
