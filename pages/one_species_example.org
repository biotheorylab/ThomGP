...
\[Z = \exp(\frac{k_{\emptyset \to A}}{k_{A \to \emptyset}} (1 - e^{-k_{A \to \emptyset} t}) (1 - e^{\theta})) (e^{-k_{A \to \emptyset} t} (e^{\theta} - 1) + 1)\]
To recover the probability distribution, expand as a power series:
