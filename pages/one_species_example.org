...
For concreteness, let us pick the initial condition of a single \(A\) at time zero.  This  means that \(e^{-\Gamma(z, 0)} = z = e^{\theta}\), i.e. \(\Gamma(\theta, 0) = -\theta\).  Hence we have \(f(z) = z + 1 + \log(z + 1)\)
