...
Since this equation is of the first order, we may solve it using the method of characteristics to obtain the general solution
\[ \Gamma (z, t) = f(e^{k_{A \to \emptyset} t} (z - 1)) - \frac{k_{\emptyset \to A}}{k_{A \to \emptyset}} z. \]
Making the change of variable $z \rightarrow e^{\theta}$, this becomes
...
The solution in the original coordinates can of course be obtained substituting $\theta \rightarrow \log(z)$ ...
