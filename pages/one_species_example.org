...
For concreteness, let us pick the initial condition of a single \(A\) at time zero, i.e. \(\Gamma(\theta, 0) = -\theta\).  Hence we have \(f(z) = \frac{k_{\emptyset \to A}}{k_{A \to \emptyset}} (z + 1) + \log(z + 1)\) and so
