...
* We can also recover the probability distribution from the one-step transition probabilities and the probability of the initial state.  Given a history $k \in H_{n}$, we would like to compute $P(\{k\})$.  Tautologously, we have $\{k\} = \{h \in H_{n} \mid }