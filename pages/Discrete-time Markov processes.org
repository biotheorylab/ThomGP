...
* We now assume that the process is _one-step Markovian_, a.k.a. _memoryless_.  This means that future values only depend upon the present value and not on past values.  Formally, given any time $0 < t  < n$ and any values $q_1, q_2, q_3 \in S$, we have
...
** As a consequence of this definition, we derive a stronger statement.  Suppose we have a sequence of times $0 < t_1 < \cdots < t_k < n$ and values $q_1, q_2, \ldots q_k \in S$.
