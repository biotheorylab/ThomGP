...
$$\begin{aligned}\sum_{i \in I} P(A \mid B_{i} \cap C) P (B_{i} | C) &= \sum_{i \in I} \frac{P(A \cap B_{i} \cap C)}{P(B \cap C)} \frac{P(B_{i} \cap C)}{P(C)} \\ &= \frac{1}{P(C)}  P(A \cap B_{i} \cap C) \\ &= \frac{P(A \cap C)}{P(C)} = P(A \mid C)\end{aligned} $$