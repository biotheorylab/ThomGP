* Here we systematically derive some basic facts for discrete-time Markov processes. For simplicity, we will restrict ourselves to finite state spaces and finite time so as to keep the exposition elementary.
...
* We now assume that the process is _one-step Markovian_, a.k.a. _memoryless_.  This means that future values only depend upon the present value and not on past values.  Formally, given any times $0 < t < t'  \le n$ and any $q \colon \{0, 1, \ldots t, t+1\} \to S$, we have
$$\begin{aligned} &P (\{h \in H_{n} \mid h(t + 1) = q(t + 1)\} \mid \{h \in H_{n} \mid h(t) = q(t)\}) = \\ &P (\{h \in H_{n} \mid h(t + 1) = q(t + 1)\} \mid \{h \in H_{n} \mid \forall j \in \{ 0, 1, \ldots, t\} \,.\, h(j) = q(j)\}) \end{aligned}$$
