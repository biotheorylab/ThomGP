...
* We now assume that the process is _one-step Markovian_, a.k.a. _memoryless_.  This means that future values only depend upon the present value and not on past values.  Formally, given any time $0 < t  < n$ and any $q \colon \{0, 1, \ldots t, t+1\} S$, we have
$$\begin{aligned} &P (\{h \in H_{n} \mid h(t + 1) = q(t + 1)}\} \mid \{h \in H_{n} \mid \forall j \in \{0, 1, \ldots, t\} . h(j) = q(j) \}) = \\ &P (\{h \in H_{n} \mid h(t + 1) = q(n + 1)}\} \mid \{h \in H_{n} \mid h(t_n) = q_n) \end{aligned}$$
