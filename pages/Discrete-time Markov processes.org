...
$$\begin{aligned} &P (\{h \in H_{n} \mid h(t') = q(t')\} \mid \{h \in H_{n} \mid h(t) = q(t)\}) = \\ &P (\{h \in H_{n} \mid h(t + 1) = q(t + 1)\} \mid \{h \in H_{n} \mid \forall j \in \{ 0, 1, \ldots, t\} \,.\, h(j) = q(j)\}) \end{aligned}$$
