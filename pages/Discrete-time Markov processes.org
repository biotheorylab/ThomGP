...
$$\begin{aligned} &T(k(t+1), t+1 \mid k(t), t) = \\ &P(\{h \in H_{n} \mid h(t+1) = k(t+1)\} \mid \} = \\ &P(\{h \in H_{n} \mid h(t+1) = k(t+1)\} \mid \{h \in H_{n} \mid \forall j \in \{0, 1, \ldots, t\} \,.\, h(j) = k(j)\}) \end{aligned}$$